...
class OrderController extends ChangeNotifier {
  ...
  Future<void> getOrderList(BuildContext? context, int offset, String status,
    ...
    ApiResponse apiResponse =
        await orderServiceInterface.getOrderList(offset, status);
  ...
  Future setDeliveryCharge(
    ...
    ApiResponse apiResponse = await orderServiceInterface.setDeliveryCharge(
        orderId, deliveryCharge, deliveryDate);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  Future<ApiResponse> assignThirdPartyDeliveryMan(BuildContext context,
    ...
    assigning = true;
    notifyListeners();
    ApiResponse apiResponse = await orderServiceInterface
        .assignThirdPartyDeliveryMan(name, trackingId, orderId);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      Provider.of<OrderDetailsController>(Get.context!, listen: false)
          .getOrderDetails(orderId.toString());
  ...
  Future<ApiResponse> editShippingAndBillingAddress({
    ...
    assigning = true;
    notifyListeners();
    ApiResponse apiResponse = await orderServiceInterface.orderAddressEdit(
        orderID: orderID,
