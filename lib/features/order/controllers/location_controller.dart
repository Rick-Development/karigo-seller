...
import 'package:karingo_seller/features/order/domain/models/place_details_model.dart';
import 'package:karingo_seller/features/order/domain/models/prediction_model.dart';
...
class LocationController with ChangeNotifier {
  ...
  TextEditingController get locationTextEditingController =>
  ...
  void updatePosition(CameraPosition? position, bool fromAddress,
    ...
    if (_updateAddAddressData) {
      ...
      try {
        if (fromAddress) {
          print("====UpdatePositionFromAddress====>>>");
          _position = Position(
              ...
              headingAccuracy: 1);
          print(
              '===PositionLat===>${_position!.latitude}====PositionLong====>${_position!.longitude}');
        } else {
        ...
        if (_changeAddress) {
          String? previousAddress = await getAddressFromGeocode(
              LatLng(position.target.latitude, position.target.longitude),
              context);
          fromAddress
              ? _address = previousAddress
              : _pickAddress = previousAddress;
