  ...
  Widget build(BuildContext context) {
    return Consumer<AuthController>(builder: (authContext, authProvider, _) {
      return Padding(
        ...
        child: Wrap(children: [
          view(getTranslated('8_or_more_character', context)!,
              authProvider.lengthCheck),
          view(getTranslated('1_number', context)!, authProvider.numberCheck),
          view(getTranslated('1_upper_case', context)!,
              authProvider.uppercaseCheck),
          view(getTranslated('1_lower_case', context)!,
              authProvider.lowercaseCheck),
          view(getTranslated('1_special_character', context)!,
              authProvider.spatialCheck),
        ]),
      );
    });
  }
