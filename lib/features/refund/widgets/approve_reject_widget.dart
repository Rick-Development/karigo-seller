...
class ApprovedAndRejectWidget extends StatefulWidget {
  ...
  @override
  State<ApprovedAndRejectWidget> createState() =>
      _ApprovedAndRejectWidgetState();
}
                  ...
                  onTap: () {
                    if (widget.refundModel!.customer == null) {
                      showCustomSnackBarWidget(
                          getTranslated(
                              'customer_account_was_deleted_you_cant_update_status',
                              context),
                          context);
                    } else {
                          ...
                          builder: (BuildContext context) {
                            return ConfirmationDialogWidget(
                                ...
                                description: getTranslated(
                                    'are_you_sure_want_to_reject', context),
                                note: noteController,
                                refund: true,
                                onYesPressed: () {
                                  if (noteController.text.trim().isEmpty) {
                                    showCustomSnackBarWidget(
                                        getTranslated('note_required', context),
                                        context);
                                  } else {
                                    refundReq.isLoading
                                        ? const Center(
                                        ...
                                        : refundReq
                                            .updateRefundStatus(
                                                context,
                                                widget.refundModel!.id,
                                                'rejected',
                                                noteController.text.trim())
                                            .then((value) {
                                            if (value.response!.statusCode ==
                                                200) {
                                              Provider.of<RefundController>(
                                                      context,
                                                      listen: false)
                      ...
                      showDialog(
                          ...
                          builder: (BuildContext context) {
                            return ConfirmationDialogWidget(
                                icon: Images.okIcon,
                                description: getTranslated(
                                    'are_you_sure_want_to_approve', context),
                                note: noteController,
                                refund: true,
                                onYesPressed: () {
                                  if (noteController.text.trim().isEmpty) {
