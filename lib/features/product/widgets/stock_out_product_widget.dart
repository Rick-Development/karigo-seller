...
import 'package:karingo_seller/common/basewidgets/product_widget.dart';
...
import 'package:karingo_seller/features/product/screens/stock_out_product_screen.dart';
import 'package:karingo_seller/features/product/widgets/stockout_product_card_widget.dart';

class StockOutProductView extends StatelessWidget {
  final bool isHome;
  final ScrollController? scrollController;
  const StockOutProductView(
      {Key? key, required this.isHome, this.scrollController})
      : super(key: key);
    ...
    scrollController.addListener(() {
      if (scrollController.position.maxScrollExtent ==
        ...
        pageSize = (Provider.of<ProductController>(context, listen: false)
                    .stockOutProductPageSize! /
                10)
            .ceil();

        if (Provider.of<ProductController>(context, listen: false).offset <
            pageSize) {
          Provider.of<ProductController>(context, listen: false).setOffset(
              Provider.of<ProductController>(context, listen: false).offset +
                  1);
          if (kDebugMode) {
                    ...
                    children: [
                      ...
                      Expanded(
                        child: TitleRowWidget(
                            title:
                                '${getTranslated('stock_out_product', context)}',
                            onTap: () => Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (_) =>
                                        const StockOutProductScreen()))),
                      ),
                    ],
                  ),
                )
