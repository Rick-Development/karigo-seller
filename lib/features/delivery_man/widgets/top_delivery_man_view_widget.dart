...
import 'package:karingo_seller/features/delivery_man/screens/top_delivery_man_screen.dart';
import 'package:karingo_seller/features/delivery_man/widgets/top_delivery_man_widget.dart';

class TopDeliveryManViewWidget extends StatelessWidget {
  final bool isMain;
  const TopDeliveryManViewWidget({Key? key, this.isMain = false})
      : super(key: key);
                  ...
                  child: Row(
                    children: [
                      SizedBox(
                          ...
                          child: Image.asset(Images.topDeliveryMan)),
                      const SizedBox(width: Dimensions.paddingSizeExtraSmall),
                      Expanded(
                        child: TitleRowWidget(
                            title:
                                '${getTranslated('top_delivery_man', context)}',
                            onTap: () => Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (_) =>
                                        const TopDeliveryMAnScreen()))),
                      ),
                    ],
                  ),
                )
              ...
              ? deliveryManList!.isNotEmpty
                  ? Padding(
                      ...
                      child: GridView.builder(
                        ...
                        itemCount: isMain && deliveryManList.length > 4
                            ? 4
                            : deliveryManList.length,
                        itemBuilder: (context, index) {
                          return TopDeliveryManWidget(
