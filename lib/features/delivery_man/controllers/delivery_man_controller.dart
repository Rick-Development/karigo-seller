...
import 'package:karingo_seller/features/delivery_man/domain/model/delivery_man_body.dart';
import 'package:karingo_seller/features/delivery_man/domain/model/delivery_man_earning_model.dart';
import 'package:karingo_seller/features/delivery_man/domain/model/delivery_man_model.dart';
import 'package:karingo_seller/features/delivery_man/domain/model/delivery_man_order_history_model.dart';
import 'package:karingo_seller/features/delivery_man/domain/model/delivery_man_review_model.dart';
import 'package:karingo_seller/features/delivery_man/domain/model/delivery_man_withdraw_detail_model.dart';
import 'package:karingo_seller/features/delivery_man/domain/model/delivery_man_withdraw_model.dart';
import 'package:karingo_seller/features/delivery_man/domain/model/order_history_log_model.dart';
import 'package:karingo_seller/features/delivery_man/domain/model/delivery_man_detail_model.dart'
    as d;
...
class DeliveryManController extends ChangeNotifier {
  ...
  final List<String> _deliveryTypeList = [
  ...
  Future<void> getDeliveryManList(Order? orderModel) async {
    ...
    ApiResponse apiResponse =
        await deliveryServiceInterface.getDeliveryManList();
    _deliveryManList = [];
    ...
    if (orderModel!.deliveryManId != null) {
      setDeliverymanIndex(
          deliveryManIds
              .indexOf(int.parse(orderModel.deliveryManId.toString())),
          false);
    }
  ...
  Future<void> deliveryManListURI(int offset, String search,
    ...
    ApiResponse apiResponse =
        await deliveryServiceInterface.deliveryManList(offset, search);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _listOfDeliveryMan = [];
      _listOfDeliveryMan!.addAll(
          TopDeliveryManModel.fromJson(apiResponse.response!.data)
              .deliveryMan!);
    } else {
  ...
  Future<void> getDeliveryManOrderList(
      BuildContext context, int offset, int? deliveryManId,
      {bool reload = true}) async {
    if (reload) {
      _deliverymanOrderList = [];
      _isLoading = true;
    }
    ApiResponse apiResponse = await deliveryServiceInterface
        .deliveryManOrderList(offset, deliveryManId);
    _deliverymanOrderList.addAll(
        DeliveryManOrderHistoryModel.fromJson(apiResponse.response!.data)
            .orders!);
    _isLoading = false;
    notifyListeners();
  }

  Future<void> getDeliveryManEarningList(
      BuildContext context, int offset, int? deliveryManId,
      {bool reload = true}) async {
    if (reload) {
    ...
    _isLoading = true;
    ApiResponse apiResponse = await deliveryServiceInterface
        .deliveryManEarningList(offset, deliveryManId);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      if (offset == 1) {
        _deliveryManEarning =
            DeliveryManEarningModel.fromJson(apiResponse.response!.data);
      } else {
        _deliveryManEarning!.totalSize =
            DeliveryManEarningModel.fromJson(apiResponse.response!.data)
                .totalSize;
        _deliveryManEarning!.offset =
            DeliveryManEarningModel.fromJson(apiResponse.response!.data).offset;
        _deliveryManEarning!.orders!.addAll(
            DeliveryManEarningModel.fromJson(apiResponse.response!.data)
                .orders!);
      }
    } else {
  ...
  Future<void> getDeliveryManDetails(int? deliveryManId) async {
    _isLoading = true;
    ApiResponse apiResponse =
        await deliveryServiceInterface.deliveryManDetails(deliveryManId);
    _deliveryManDetails =
        d.DeliveryManDetailsModel.fromJson(apiResponse.response!.data);
    _isLoading = false;
    notifyListeners();
  }

  Future<void> getTopDeliveryManList(BuildContext context,
      {bool reload = true}) async {
    if (reload) {
      _isLoading = false;
      _topDeliveryManList = [];
    ...
    _isLoading = true;
    ApiResponse apiResponse =
        await deliveryServiceInterface.getTopDeliveryManList();
    _topDeliveryManList!.addAll(
        TopDeliveryManModel.fromJson(apiResponse.response!.data).deliveryMan!);
    _isLoading = false;
    notifyListeners();
  }

  Future<void> getDeliveryManOrderHistoryLogList(
      BuildContext context, int? orderId) async {
    _isLoading = true;
    _changeLogList = [];
    _changeLogList =
        await deliveryServiceInterface.getDeliverymanOrderHistoryLog(orderId);
    _isLoading = false;
    notifyListeners();
  }
  ...
  Future<void> assignDeliveryMan(
      BuildContext context, int? orderId, int? deliveryManId) async {
    _isLoading = true;
    ResponseModel responseModel = await deliveryServiceInterface
        .assignDeliveryMan(orderId, deliveryManId);
    if (responseModel.isSuccess) {
      ...
      showCustomSnackBarWidget(
          getTranslated('delivery_man_assign_successfully', Get.context!),
          Get.context!,
          isToaster: true,
          isError: false);
    }
  ...
  Future<void> deliveryManStatusOnOff(
      BuildContext context, int? id, int status) async {
    _isLoading = true;
    ResponseModel responseModel =
        await deliveryServiceInterface.deliveryManStatusOnOff(id, status);
    if (responseModel.isSuccess) {
  ...
  Future<void> collectCashFromDeliveryMan(
      BuildContext context, int? deliveryManId, String amount) async {
    _isLoading = true;
    notifyListeners();
    ResponseModel responseModel = await deliveryServiceInterface
        .collectCashFromDeliveryMan(deliveryManId, amount);
    _isLoading = false;
    if (responseModel.isSuccess) {
      Navigator.pop(Get.context!);
      getDeliveryManDetails(deliveryManId);
      showCustomSnackBarWidget(
          getTranslated('amount_collected_from_deliveryman', Get.context!),
          Get.context!,
          isToaster: true);
    } else {
  ...
  Future<void> deleteDeliveryMan(
      BuildContext context, int? deliveryManId) async {
    ResponseModel responseModel =
        await deliveryServiceInterface.deleteDeliveryMan(deliveryManId);
    if (responseModel.isSuccess) {
  ...
  Future<ResponseModel> addNewDeliveryMan(
      BuildContext context, DeliveryManBody deliveryManBody,
      {bool isUpdate = false}) async {
    _isLoading = true;
    notifyListeners();
    ResponseModel responseModel =
        await deliveryServiceInterface.addNewDeliveryMan(
            _profileImage,
            _identityImages,
            deliveryManBody,
    ...
    if (responseModel.isSuccess) {
      ...
      isUpdate
          ? showCustomSnackBarWidget(
              getTranslated("delivery_man_updated_successfully", Get.context!),
              Get.context!,
              isError: false)
          : showCustomSnackBarWidget(
              getTranslated("delivery_man_added_successfully", Get.context!),
              Get.context!,
              isError: false);
    }
    _isLoading = false;
  ...
  Future<void> getDeliveryManWithdrawDetails(
  ...
  Future<void> getDeliveryManWithdrawList(int offset, String status,
      {bool reload = true}) async {
    if (reload) {
      _withdrawList = [];
      _isLoading = true;
    ...
    ApiResponse apiResponse =
        await deliveryServiceInterface.deliveryManWithdrawList(offset, status);
    _withdrawList.addAll(
        DeliveryManWithdrawModel.fromJson(apiResponse.response!.data)
            .withdraws!);
    _isLoading = false;
    notifyListeners();
  }

  Future<void> getDeliveryManReviewList(
      BuildContext context, int offset, int? id,
      {bool reload = true}) async {
    if (reload) {
    ...
    ApiResponse apiResponse =
        await deliveryServiceInterface.getDeliveryManReviewList(offset, id);
    _deliveryManReviewList.addAll(
        DeliveryManReviewModel.fromJson(apiResponse.response!.data).reviews!);
    _isLoading = false;
    notifyListeners();
  }

  Future<void> deliveryManWithdrawApprovedDenied(BuildContext context, int? id,
      String note, int approved, int? index) async {
    _isLoading = true;
    ResponseModel responseModel = await deliveryServiceInterface
        .deliveryManWithdrawApprovedDenied(id, note, approved);
    if (responseModel.isSuccess) {
      Navigator.pop(Get.context!);
  ...
  Future<void> getDeliveryCollectedCashList(
      BuildContext context, int? deliveryManId, int offset) async {
    ApiResponse apiResponse = await deliveryServiceInterface
        .getDeliveryManCollectedCashList(deliveryManId, offset);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      if (offset == 1) {
        _collectedCashModel = null;
        _collectedCashModel =
            CollectedCashModel.fromJson(apiResponse.response!.data);
      } else {
        _collectedCashModel!.totalSize =
            CollectedCashModel.fromJson(apiResponse.response!.data).totalSize;
        _collectedCashModel!.offset =
            CollectedCashModel.fromJson(apiResponse.response!.data).offset;
        _collectedCashModel!.collectedCash!.addAll(
            CollectedCashModel.fromJson(apiResponse.response!.data)
                .collectedCash!);
      }
    } else {
