...
import 'package:karingo_seller/features/chat/domain/models/message_model.dart';
...
class ChatController extends ChangeNotifier {
  ...
  Future<void> getChatList(BuildContext context, int offset,
    ...
    ApiResponse apiResponse = await chatServiceInterface.getChatList(
        _userTypeIndex == 0 ? 'customer' : 'delivery-man', offset);
  ...
  Future<void> searchedChatList(BuildContext context, String search) async {
    ApiResponse apiResponse = await chatServiceInterface.searchChat(
        _userTypeIndex == 0 ? 'customer' : 'delivery-man', search);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  Future<http.StreamedResponse> sendMessage(
    ...
    http.StreamedResponse response = await chatServiceInterface.sendMessage(
        messageBody,
        _userTypeIndex == 0 ? 'customer' : 'delivery-man',
        _pickedImageFiles);
