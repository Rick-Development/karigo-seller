...
import 'package:karingo_seller/features/pos/domain/models/invoice_model.dart';
...
import 'package:karingo_seller/features/pos/domain/models/customer_model.dart';
...
import 'package:karingo_seller/features/pos/screens/invoice_screen.dart';
import 'package:karingo_seller/features/pos/widgets/product_variation_selection_dialog_widget.dart';
...
class CartController extends ChangeNotifier {
  ...
  void applyCouponCodeAndExtraDiscount(BuildContext context) {
    ...
    if (_extraDiscountAmount > _amount) {
      showCustomSnackBarWidget(
          getTranslated('discount_cant_greater_than_order_amount', context),
          context,
          isToaster: true);
    } else {
      _customerCartList[_customerIndex].extraDiscount = _extraDiscountAmount;
      showCustomSnackBarWidget(
          getTranslated('extra_discount_added_successfully', context), context,
  ...
  void addToCart(BuildContext context, CartModel cartModel,
    ...
    if (_customerCartList[_customerIndex].cart!.any((e) =>
        (e.product!.id == cartModel.product!.id &&
            e.variant == cartModel.variant))) {
  ...
  void addToCartListForUser(TemporaryCartListModel cartList,
      {bool clear = false}) {
    if (_customerCartList.isEmpty) {
      _customerIds = [];
    }

    if (_customerCartList
        .any((e) => e.userId == cartList.userId && cartList.userId != 0)) {
      ...
      showCustomSnackBarWidget(
          getTranslated('new_order_added_successfully', Get.context!),
          Get.context!,
          isError: false,
  ...
  void setQuantity(BuildContext context, bool isIncrement, int? index,
    ...
    if (isIncrement) {
      if (_customerCartList[_customerIndex]
          ...
          _customerCartList[_customerIndex].cart![index].product!.productType ==
              'physical') {
        _amount = 0;
        _customerCartList[_customerIndex].cart![index].quantity =
      ...
      } else if (_customerCartList[_customerIndex]
              .cart![index]
              .product!
              .productType ==
          'digital') {
    ...
    for (int index = 0;
      ...
      if (_customerCartList[_customerIndex].cart![index].product!.id ==
        ...
        if (decreaseQuantity) {
          ...
          showCustomSnackBarWidget(
              '${getTranslated('added_cart_successfully', context)} ${_customerCartList[_customerIndex].cart![index].quantity} ${getTranslated('items', context)}',
              context,
              isToaster: true,
              isError: false);
        }
  ...
  Future<void> getCouponDiscount(BuildContext context, String couponCode,
      int? userId, double orderAmount) async {
    ApiResponse response = await cartServiceInterface.getCouponDiscount(
    ...
    if (response.response!.statusCode == 200) {
      ...
      showCustomSnackBarWidget(
          'You got ${_couponCodeAmount.toString()} discount', Get.context!,
          isToaster: true, isError: false);
  ...
  Future<ApiResponse> placeOrder(
    ...
    if (response.response!.statusCode == 200 &&
      ...
      if (_customerIds.isNotEmpty) {
        ...
        setCustomerInfo(_customerCartList[_customerIndex].userId,
            _customerCartList[_customerIndex].customerName, '', true);
      }
      Navigator.push(
          Get.context!,
          MaterialPageRoute(
  ...
  Future<void> searchCustomer(BuildContext context, String searchName) async {
    ...
    _isGetting = true;
    ApiResponse response =
        await cartServiceInterface.customerSearch(searchName);
    if (response.response!.statusCode == 200) {
      _searchedCustomerList = [];
      _searchedCustomerList!
          .addAll(CustomerModel.fromJson(response.response!.data).customers!);
