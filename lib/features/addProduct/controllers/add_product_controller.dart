...
import 'package:karingo_seller/features/addProduct/domain/models/attr.dart';
...
import 'package:karingo_seller/features/addProduct/domain/models/brand_model.dart';
import 'package:karingo_seller/features/addProduct/domain/models/category_model.dart';
import 'package:karingo_seller/features/addProduct/domain/models/product_image_model.dart';
import 'package:karingo_seller/features/addProduct/domain/services/add_product_service_interface.dart';
import 'package:karingo_seller/features/splash/domain/models/config_model.dart';
import 'package:karingo_seller/features/addProduct/domain/models/edt_product_model.dart';
...
class AddProductController extends ChangeNotifier {
  ...
  void setStringImage(int index, String image, String colorCode) {
    ...
    withColor[index].colorImage =
        ColorImage(color: colorCode, imageName: image);
  }
  ...
  Future<void> getProductImage(String id) async {
    ...
    if (apiResponse.response != null &&
      ...
      if (productImagesModel!.colorImage!.isNotEmpty) {
        ...
        for (ColorImage img in productImagesModel!.colorImage!) {
          if (img.imageName != null) {
            productReturnImage?.add(img.imageName);
          }
          if (img.color != null) {
            imagesWithColorForUpdate.add(img.imageName ?? '');
          ...
          if (withColor.isNotEmpty) {
            for (int index = 0; index < withColor.length; index++) {
              ...
              if (bb == img.color) {
                setStringImage(index, img.imageName ?? '', img.color ?? '');
              }
            }
          }
          if (img.color == null) {
            imagesWithoutColor.add(img.imageName!);
          }
        }
      } else {
        imagesWithoutColor.addAll(productImagesModel!.images!);
        productReturnImage?.addAll(productImagesModel!.images!);
      }
      log("===>ColorImgage Length==> ${colorImageObject.length}");
    } else {
      _isLoading = false;
      ApiChecker.checkApi(apiResponse);
    }
